...
export function makeAppData(overrides = {}) {
  ...
  return {
    ...
    strava: {
      startConnect: vi.fn(),
      sync: vi.fn().mockResolvedValue({ synced: 0, total: 0 }),
    ...
    hierarchicalPlan: {
      ...
      deleteWorkout: vi.fn().mockResolvedValue(SAMPLE_HIERARCHICAL_PLAN),
    ...
    seasonPlans: {
      ...
      addRaceToPlan: vi.fn().mockResolvedValue({}),
      updateRaceInPlan: vi.fn().mockResolvedValue({}),
      removeRaceFromPlan: vi.fn().mockResolvedValue({}),
      reorderRacesInPlan: vi.fn().mockResolvedValue({}),
    },
